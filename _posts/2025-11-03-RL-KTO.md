...
description: "Panoramica di KTO (Kahneman-Tversky Optimization) ed esempi per creazione di dataset"
...
Per usare KTO con [TRL](https://github.com/huggingface/trl){:target="_blank"} usa la classe [KTOTrainer](https://huggingface.co/docs/trl/v0.24.0/en/kto_trainer#trl.KTOTrainer){:target="_blank"} e KTOConfig.  
...
Tornando all'esempio del dataset costruito dalle chat telegram del progetto [LLM "Chat Like Me"](https://bigghis.github.io/posts/2025-10-16-LLM-CHAT-LIKE-ME/), possiamo fare diversi ragionamenti per generare dataset di preferenze negative:

**Base model con sampling aggressivo**: generando risposte dal modello base (ante SFT) con parametri che aumentano la variabilità delle risposte (parametri di temperatura alta, top-p basso etc.)
...
**Contaminazione con altri dataset o/e template generici/formali**: usando risposte di altri interlocutori/persone su prompt simili, oppure frasi standard, neutrali o eccessivamente formali non in linea con lo stile cercato.
```json
{
  "prompt": "Come va?",
  "completion": "La ringrazio per il suo messaggio. Valuterò attentamente.",
